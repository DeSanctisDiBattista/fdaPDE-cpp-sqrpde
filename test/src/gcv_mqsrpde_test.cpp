...
// test 2
//    domain:       unit square
//    sampling:     locations != nodes  (+ minimum distance criterion)
//    penalization: constant PDE coefficients 
//    covariates:   yeas
//    BC:           no
//    order FE:     1
...
TEST(gcv_msqrpde_test2, pde_semiparametric_samplingatlocations_spaceonly_gridexact) {
    ...
    for(double x = -6.0; x <= -4.5; x += lambdas_step) lambdas_1.push_back(std::pow(10, x));
    for(double x = -6.0; x <= -4.5; x += lambdas_step) lambdas_2.push_back(std::pow(10, x));
    for(double x = -6.0; x <= -4.5; x += lambdas_step) lambdas_3.push_back(std::pow(10, x));
    for(double x = -6.0; x <= -4.0; x += lambdas_step) lambdas_5.push_back(std::pow(10, x)); 

    for(double x = -6.5; x <= -3.5; x += lambdas_step) lambdas_10.push_back(std::pow(10, x)); 
    ...
    for(double x = -6.5; x <= -3.5; x += lambdas_step) lambdas_90.push_back(std::pow(10, x));

    for(double x = -6.5; x <= -3.5; x += lambdas_step) lambdas_91.push_back(std::pow(10, x));
    for(double x = -6.5; x <= -3.5; x += lambdas_step) lambdas_92.push_back(std::pow(10, x));
    for(double x = -7.0; x <= -3.5; x += lambdas_step) lambdas_93.push_back(std::pow(10, x));
    for(double x = -7.0; x <= -3.5; x += lambdas_step) lambdas_94.push_back(std::pow(10, x));
    for(double x = -7.0; x <= -3.5; x += lambdas_step) lambdas_95.push_back(std::pow(10, x));
    for(double x = -7.0; x <= -3.5; x += lambdas_step) lambdas_96.push_back(std::pow(10, x));
    for(double x = -7.0; x <= -3.0; x += lambdas_step) lambdas_97.push_back(std::pow(10, x));
    for(double x = -6.5; x <= -3.5; x += lambdas_step) lambdas_98.push_back(std::pow(10, x)); 
    for(double x = -6.5; x <= -3.0; x += lambdas_step) lambdas_99.push_back(std::pow(10, x));
...
//     // data parameters
//     std::vector<unsigned int> max_reps = {50};  
...
//             std::cout << "substring summary=" << gcv_summary.substr(gcv_summary.length() - 2) << std::endl;
...
//                                     } else{
//                                         std::cout << "------------------KCV computation-----------------" << std::endl;
...
//                                         KCV kcv(n_folds);
//                                         auto fit_kcv = kcv.fit(model_cv, lambdas_mat, RMSE(model_cv));
...
//                                 // RMSE
//                                 DMatrix<double> f_true = read_csv<double>(R_path + "/true/mean_true.csv");
//                                 DMatrix<double> fn_true = read_csv<double>(R_path + "/true/mean_true_loc_" + nxx_loc + "_" + data_type + ".csv");
...
//                                         sol = collapse_rows(model_rmse.Psi()*model_rmse.f(), model_rmse.unique_locs_flags(), model_rmse.num_unique_locs()) + model_rmse.X_reduced()*model_rmse.beta(); 
//                                         sol_true = collapse_rows(fn_true, model_rmse.unique_locs_flags(), model_rmse.num_unique_locs()) + model_rmse.X_reduced()*beta_true;  
...
//                                     DMatrix<double> fn_true = read_csv<double>(R_path + "/true/f" + alpha_string + "_true_loc_" + nxx_loc + "_" + data_type + ".csv");
...
//                                         if(has_covariates){
//                                             sol = collapse_rows(model_rmse.Psi()*model_rmse.f(), model_rmse.unique_locs_flags(), model_rmse.num_unique_locs()) + model_rmse.X_reduced()*model_rmse.beta(); 
//                                             sol_true = fn_true +  model_rmse.X_reduced()*beta_true;  
//                                         } else{
//                                             sol = model_rmse.f(); 
//                                             sol_true = f_true; 
//                                         }
...
//                                 if(has_covariates){
//                                     // note: here data_type = "data" => no quantità reduced                                   
//                                     sol = model_rmse.Psi()*model_rmse.f() + model_rmse.X()*model_rmse.beta(); 
//                                     sol_true = fn_true +  model_rmse.X()*beta_true;  
//                                 } else{
//                                     sol = model_rmse.f(); 
//                                     sol_true = f_true; 
//                                 }
