...
//     const std::string day_chosen = "11"; 
//     const std::string K = "1";    // maximum number of repeated observations per each location 
...
//     std::vector<std::string> gcv_summaries = {"_I_appr",  "_II_appr", "_III_appr"};   // "_I_appr",  "_II_appr", "_III_appr", "_IV_appr
...
//             } else{
//                 solutions_path = path + "/results/MQSRPDE/obs_rip" + K + "/" + month + "/day_" + day_chosen + "/" + model_type + "/eps_" + eps_string + "/" + covariate_type;
//             }
...
//     if(est_type == "quantile"){
//         for(double x = -6.8; x <= 0.0; x += 0.5) lambdas_1_5.push_back(std::pow(10, x)); 
//         for(double x = -6.0; x <= 0.0; x += 0.5) lambdas_10_25.push_back(std::pow(10, x));
//         for(double x = -9.5; x <= -2.5; x += 0.5) lambdas_30_70.push_back(std::pow(10, x)); 
//         for(double x = -9.5; x <= -3.0; x += 0.5) lambdas_75_90.push_back(std::pow(10, x)); 
//         for(double x = -9.5; x <= -3.0; x += 0.5) lambdas_95_99.push_back(std::pow(10, x));
//     }

//     // define spatial domain and regularizing PDE
//     const std::string mesh_type = "mesh_lombardia_convex_hull_fine";  // mesh_lombardia_convex_hull mesh_lombardia_convex_hull_fine
//     if(mesh_type == "mesh_lombardia_convex_hull_fine" && est_type == "quantile")
//         std::cout << "ATT: RUNNING QUANTILE WITH A FINE MESH" << std::endl; 
//     if(mesh_type == "mesh_lombardia_convex_hull" && est_type == "mean")
//         std::cout << "ATT: RUNNING SRPDE WITH A COARSE MESH" << std::endl; 
    
//     MeshLoader<Triangulation<2, 2>> domain(mesh_type);
...
//     if(model_type == "param"){
//         if(covariate_type == "dens.new_log.el.orig"){
//             X = read_csv<double>(path_data + "/X_dens.new_log.elev.original_rip" + K + ".csv");
//         } else{
//             X = read_csv<double>(path_data + "/X_" + covariate_type + "_rip" + K + ".csv");
//         }
//     }
...
TEST(case_study_mqsrpde_run_obs_rip, NO2) {
    ...
    std::vector<std::string> gcv_summaries = {"_I_appr"};   // "_I_appr",  "_II_appr", "_III_appr", "_IV_appr
