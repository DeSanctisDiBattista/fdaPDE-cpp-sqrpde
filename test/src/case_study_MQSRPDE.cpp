...
TEST(case_study_mqsrpde_gcv, NO2_restricted) {
    ...
    const std::string  cascading_model_type = "parametric";  // "parametric" "nonparametric" "parametric2"
    ...
    DMatrix<double> u; 
    if(cascading_model_type == "nonparametric"){
        b_data  = read_csv<double>(path_data + "/" + mesh_type_param_casc + "/b_" + u_string + "_opt_nonparametric.csv");
        u = read_csv<double>(path_data + "/" + mesh_type_param_casc + "/u_" + u_string + "_opt_nonparametric.csv");
    }  
...
TEST(case_study_mqsrpde_run, NO2_restricted) {
   ...
   // std::vector<unsigned int> num_fpirls_iter_per_alpha = {10, 10}; 
    ...
    const unsigned int max_it_convergence_loop = 100;       // numero massimo iterazioni MM 
