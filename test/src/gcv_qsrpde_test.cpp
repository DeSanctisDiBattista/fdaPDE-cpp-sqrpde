...
TEST(gcv_sqrpde_test_cv, pde_nonparametric_samplingatlocations_spaceonly_gridexact) {
    ...
    // path test  
    std::string R_path = "/mnt/c/Users/marco/OneDrive - Politecnico di Milano/Corsi/PhD/Codice/models/QSRPDE/Tests_gcv"; 
    ...
    bool compute_gcv = true;     // if you want to compute either gcv, gacv or gacv*
  
    // correct gcv? 
    std::vector<std::string> score_types = {"k-fold"};   // "gcv" "gacv" "gacv_star" "k-fold"
    std::size_t n_folds = 5;

    const unsigned int n_sim = 20; 

    // define domain
    MeshLoader<Triangulation<2, 2>> domain("unit_square_15");
    ...
    for(auto sim = 1; sim <= n_sim; ++sim){
        ...
        for(auto alpha : alphas){
            ...
            std::cout << "check num lambdas = " << lambdas.size() << std::endl; 
                ...
                for(auto lambda_selection_type : lambda_selection_types){
                    for(auto score_type : score_types){
                        ...
                        if(score_type == "k-fold"){
                            ...
                            KCV kcv(n_folds);
                            auto fit_kcv = kcv.fit(model_cv, lambdas_mat, RMSE(model_cv, alpha));
                            //auto KCV_ = fdapde::calibration::KCV {n_folds}(lambdas_mat, RMSE());
