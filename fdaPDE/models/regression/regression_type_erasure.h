...
struct RegressionModel__ {
    ...
    using fn_ptrs = fdapde::mem_fn_ptrs<
      ...
      &M::norm, &M::y, &M::T, &M::lmbQ, &M::has_covariates, &M::nan_mask, &M::set_mask, &M::X, &M::lmbQ_reduced, &M::invA_reduced>;
    ...
    decltype(auto) lmbQ_reduced(const DMatrix<double>& x) const { return invoke<DMatrix<double>, 20>(*this, x); }
    decltype(auto) invA_reduced() const { return invoke<const fdapde::SparseLU<SpMatrix<double>>&  , 21>(*this); }
    decltype(auto) U_reduced()       const { return invoke<const DMatrix<double>&   ,22>(*this); }
    decltype(auto) V_reduced()       const { return invoke<const DMatrix<double>&   , 23>(*this); }
    decltype(auto) XtWX_reduced()    const { return invoke<const DMatrix<double>&   , 24>(*this); }
};
